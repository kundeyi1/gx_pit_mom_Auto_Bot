...
# ── 中信行业 → TDX 板块代码映射 ─────────────────────────────────────
# 每个 CITIC 行业映射到一个或多个 TDX 板块代码（优先使用第一个可用代码）。
# 列名使用 CITIC 标准名称，数据来源为 TDX 通达信板块指数。
# 标注 [近似] 表示 TDX 板块与 CITIC 行业不完全对应，仅为最佳可用代理。

# ── 一级行业映射 (28 个 CITIC 一级 → TDX 代码) ──
_CITIC_LEVEL1_MAP: dict[str, list[str]] = {
    "石油石化":          ["880302"],            # 石油
    "煤炭":              ["880301"],            # 煤炭
    "有色金属":          ["880303"],            # 有色
    "电力及公用事业":    ["880305"],            # 电力 [近似]
    "钢铁":              ["880311"],            # 矿物制品 [近似: TDX 钢铁 880309 不可用]
    "基础化工":          ["880313"],            # 化工原料 [近似]
    "建筑":              ["880476"],            # 建筑
    "建材":              ["880477", "880321"],  # 建材, 兜底水泥
    "轻工制造":          ["880326", "880318"],  # 轻工机械 [近似], 兜底造纸
    "机械":              ["880437", "880330"],  # 通用机械, 兜底专用机械
    "电力设备及新能源":  ["880431", "880305"],  # 光伏 [近似], 兜底电力
    "国防军工":          ["880444"],            # 国防军工
    "汽车":              ["880335"],            # 汽车整车
    "商贸零售":          ["880406", "880344"],  # 商业连锁 [近似], 兜底商贸代理
    "消费者服务":        ["880340", "880338"],  # 旅游景点 [近似], 兜底文教休闲
    "家电":              ["880424"],            # 智能家居 [近似: TDX 家用电器 880358 不可用]
    "纺织服装":          ["880327"],            # 纺织机械 [近似: TDX 纺织/服饰均不可用]
    "医药":              ["880400"],            # 医药
    "食品饮料":          ["880351", "880355"],  # 食品, 兜底软饮料
    "农林牧渔":          ["880360"],            # 农林牧渔
    "银行":              ["880471"],            # 银行
    "非银行金融":        ["880472", "880474"],  # 证券 [近似], 兜底多元金融
    "房地产":            ["880482"],            # 房地产
    "交通运输":          ["880366", "880367"],  # 运输服务 [近似], 兜底港口
    "电子":              ["880491", "880492"],  # 半导体 [近似], 兜底元器件
    "通信":              ["880454", "880453"],  # 通信设备, 兜底 5G概念
    "计算机":            ["880494", "880497"],  # 互联网 [近似], 兜底信创
    "传媒":              ["880418"],            # 传媒娱乐
}

# ── 二级行业映射 (108 个 CITIC 二级 → TDX 代码) ──
# 对于无法在 TDX 中找到对应板块的 CITIC 二级行业，使用其父级一级行业代码
_CITIC_LEVEL2_MAP: dict[str, list[str]] = {
    # 石油石化 (L1: 880302)
    "石油开采Ⅱ":        ["880302"],            # 石油 (父级)
    "石油化工":          ["880310"],            # 石油加工
    "油服工程":          ["880302"],            # 石油 (父级)
    ...
    "煤炭化工":          ["880313"],            # 化工原料 [近似]
    # 有色金属 (L1: 880303)
    "贵金属":            ["880520", "880303"],  # 黄金概念, 兜底有色
    "工业金属":          ["880303"],            # 有色
    "稀有金属":          ["880516", "880303"],  # 稀土永磁, 兜底有色
    # 电力及公用事业 (L1: 880305)
    "发电及电网":        ["880305"],            # 电力
    "环保及公用事业":    ["880308"],            # 环保
    # 钢铁 (L1: 880311 代理)
    ...
    "特材":              ["880311"],            # 矿物制品 [近似]
    # 基础化工 (L1: 880313)
    "农用化工":          ["880314", "880313"],  # 农药化肥 (但不可用), 兜底化工原料
    "化学纤维":          ["880304", "880313"],  # 化纤 (但不可用), 兜底化工原料
    "化学原料":          ["880313"],            # 化工原料
    "其他化学制品Ⅱ":    ["880313"],            # 化工原料 (父级)
    "塑料及制品":        ["880315", "880313"],  # 塑料 (但不可用), 兜底
    "橡胶及制品":        ["880316", "880313"],  # 橡胶 (但不可用), 兜底
    # 建筑 (L1: 880476)
    "建筑施工":          ["880476"],            # 建筑
    "建筑装修Ⅱ":        ["880324"],            # 装修装饰
    "建筑设计及服务Ⅱ":  ["880476"],            # 建筑 (父级)
    # 建材 (L1: 880477)
    "结构材料":          ["880477"],            # 建材
    "装饰材料":          ["880477", "880319"],  # 建材, 兜底玻璃
    "专用材料Ⅱ":        ["880477"],            # 建材 (父级)
    # 轻工制造 (L1: 880326)
    "造纸Ⅱ":            ["880318"],            # 造纸
    "包装印刷":          ["880339"],            # 广告包装 [近似]
    "家居":              ["880337"],            # 家居用品
    "文娱轻工Ⅱ":        ["880338"],            # 文教休闲
    "其他轻工Ⅱ":        ["880326"],            # 轻工机械 (父级)
    # 机械 (L1: 880437)
    "工程机械Ⅱ":        ["880325"],            # 工程机械
    "专用机械":          ["880330"],            # 专用机械
    "通用设备":          ["880437"],            # 通用机械
    "运输设备":          ["880333", "880438"],  # 运输设备 (但不可用), 兜底工业机械
    "仪器仪表Ⅱ":        ["880332", "880438"],  # 电器仪表 (但不可用), 兜底
    "金属制品Ⅱ":        ["880311"],            # 矿物制品 [近似]
    # 电力设备及新能源 (L1: 880431)
    "电气设备":          ["880439", "880431"],  # 高端装备 [近似], 兜底光伏
    "电源设备":          ["880426", "880431"],  # 充电桩, 兜底光伏
    "新能源动力系统":    ["880431"],            # 光伏
    # 国防军工 (L1: 880444)
    "航空航天":          ["880547", "880444"],  # 商业航天, 兜底国防军工
    "兵器兵装Ⅱ":        ["880444"],            # 国防军工
    "其他军工Ⅱ":        ["880444"],            # 国防军工
    # 汽车 (L1: 880335)
    "乘用车Ⅱ":          ["880335"],            # 汽车整车
    "商用车":            ["880335"],            # 汽车整车 (父级)
    "汽车零部件Ⅱ":      ["880336"],            # 汽车配件
    "汽车销售及服务Ⅱ":  ["880335"],            # 汽车整车 (父级)
    "摩托车及其他Ⅱ":    ["880334", "880335"],  # 摩托车 (但不可用), 兜底
    # 商贸零售 (L1: 880406)
    "一般零售":          ["880406"],            # 商业连锁
    "贸易Ⅱ":            ["880344"],            # 商贸代理
    "专营连锁":          ["880406"],            # 商业连锁
    "电商及服务Ⅱ":      ["880410"],            # 电子商务
    "专业市场经营Ⅱ":    ["880406"],            # 商业连锁 (父级)
    # 消费者服务 (L1: 880340)
    "旅游及休闲":        ["880340"],            # 旅游景点
    "酒店及餐饮":        ["880341", "880340"],  # 酒店餐饮 (但不可用), 兜底
    "教育":              ["880530", "880338"],  # 职业教育, 兜底文教休闲
    "综合服务":          ["880340"],            # 旅游景点 (父级)
    # 家电 (L1: 880424)
    "白色家电Ⅱ":        ["880424"],            # 智能家居 [近似]
    "黑色家电Ⅱ":        ["880424"],            # 智能家居 [近似]
    "小家电Ⅱ":          ["880424"],            # 智能家居 [近似]
    "照明电工及其他":    ["880425", "880424"],  # 智能电网, 兜底智能家居
    "厨房电器Ⅱ":        ["880424"],            # 智能家居 [近似]
    # 纺织服装 (L1: 880327)
    "纺织制造":          ["880327"],            # 纺织机械 [近似]
    "品牌服饰":          ["880327"],            # 纺织机械 [近似]
    # 医药 (L1: 880400)
    ...
    # 非银行金融 (L1: 880472)
    "证券Ⅱ":            ["880472"],            # 证券
    "保险Ⅱ":            ["880473", "880472"],  # 保险, 兜底证券
    "多元金融":          ["880474"],            # 多元金融
    # 房地产 (L1: 880482)
    "房地产开发和运营":  ["880482"],            # 房地产
    "房地产服务":        ["880483", "880482"],  # 物业管理, 兜底房地产
    "资产管理Ⅱ":        ["880482"],            # 房地产 (父级) [近似]
    "多领域控股Ⅱ":      ["880482"],            # 房地产 (父级) [近似]
    "新兴金融服务Ⅱ":    ["880474"],            # 多元金融 [近似]
    # 交通运输 (L1: 880366)
    "公路铁路":          ["880368"],            # 路桥
    "物流":              ["880366"],            # 运输服务 [近似]
    "航运港口":          ["880367"],            # 港口
    "航空机场":          ["880369", "880366"],  # 空运, 兜底运输服务
    # 电子 (L1: 880491)
    "半导体":            ["880491"],            # 半导体
    "元器件":            ["880492"],            # 元器件
    "光学光电":          ["880493", "880492"],  # 消费电子 [近似], 兜底元器件
    "消费电子":          ["880493"],            # 消费电子
    "其他电子零组件Ⅱ":  ["880492"],            # 元器件 (父级)
    # 通信 (L1: 880454)
    "电信运营Ⅱ":        ["880452"],            # 电信运营
    "通信设备制造":      ["880454"],            # 通信设备
    "增值服务Ⅱ":        ["880494"],            # 互联网 [近似]
    "通讯工程服务":      ["880454"],            # 通信设备 (父级)
    # 计算机 (L1: 880494)
    "计算机设备":        ["880496", "880494"],  # IT设备 (但不可用), 兜底互联网
    "计算机软件":        ["880494", "880497"],  # 互联网 [近似], 兜底信创
    "云服务":            ["880451", "880494"],  # 云计算 (但不可用), 兜底
    "产业互联网":        ["880494"],            # 互联网
    # 传媒 (L1: 880418)
    "媒体":              ["880418"],            # 传媒娱乐
    "广告营销":          ["880339"],            # 广告包装
    "文化娱乐":          ["880418"],            # 传媒娱乐
    "互联网媒体":        ["880494"],            # 互联网
    "综合Ⅱ":            ["880460", "880494"],  # 数据中心, 兜底互联网 [近似]
}
...
def fetch_sector_data(fetcher, level: int = 1) -> pd.DataFrame:
    """拉取行业板块指数 close 价格宽表, 使用 CITIC→TDX 映射.

    列名使用 CITIC 标准行业名称, 数据来自 TDX 通达信板块指数.
    ...
    -------
    pd.DataFrame
        close 价格宽表, index=date, columns=CITIC行业名称.
    """
    citic_map = _CITIC_LEVEL1_MAP if level == 1 else _CITIC_LEVEL2_MAP
    label = f"{'一级' if level == 1 else '二级'}行业 (CITIC→TDX 映射)"

    _log(f"[TDX] 拉取{label} ({len(citic_map)} 个 CITIC 行业)")
    ...
    for i, (citic_name, tdx_codes) in enumerate(citic_map.items()):
        # 尝试 tdx_codes 中的代码, 使用第一个可用的
        close_series = None
        used_code = None

        for tdx_code in tdx_codes:
            try:
                df = fetcher.fetch_index_daily_full(tdx_code)
                if df is not None and not df.empty and "close" in df.columns:
                    df["_d"] = pd.to_datetime(df["date"]).dt.normalize()
                    close_series = df.set_index("_d")["close"]
                    close_series = close_series[~close_series.index.duplicated(keep="last")]
        ...
        if close_series is not None and not close_series.empty:
            close_series.name = citic_name
            all_series[citic_name] = close_series
            success += 1
        else:
            fail_codes += 1
        ...
        # 进度
        if (i + 1) % 20 == 0:
            _log(f"    进度: {i+1}/{len(citic_map)} (成功 {success}, 失败 {fail_codes})")

    _log(f"  CITIC 映射拉取完成: 成功 {success}/{len(citic_map)}, 失败 {fail_codes}")
